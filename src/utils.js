...
function getSpeechOutputFor(lang, voiceNames, name) {
    if (voiceNames[lang]) {
        ...
        return '<voice name="' + voiceName + '"><lang xml:lang="' + lang + '">' + iAm + ' ' + name + '.</lang></voice>';
    }
    logger.error('lang not found: ' + lang);
};

exports.getFemaleSpeechOutputFor = (lang) => {
    return getSpeechOutputFor(lang, femaleVoiceNames, 'Alexa');
};

exports.getMaleSpeechOutputFor = (lang) => {
    return getSpeechOutputFor(lang, maleVoiceNames, 'Alex');
};
